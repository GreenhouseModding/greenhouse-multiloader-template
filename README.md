...
- Platform Helpers are done without services, instead being added to the common class within the Fabric mod init and
  NeoForge init.
	- Make sure to keep Fabric load order in mind whilst using the platform helper. Do not add any methods that could be called before mod init inside the helper.
